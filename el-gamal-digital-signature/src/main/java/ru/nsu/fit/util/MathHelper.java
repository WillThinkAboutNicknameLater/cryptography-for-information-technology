...
public class MathHelper {
    ...
    public static int generateCoPrimeNumber(int number, int from, int through) {
        int candidate = 0;
        boolean areCoPrimeNumbers = false;

        while (!areCoPrimeNumbers) {
            candidate = Randomizer.generateNumber(from, through);
            EuclidAlgorithmResponse euclidResponse = MathHelper.calculateEuclidAlgorithm(candidate, number);
            if (euclidResponse.gcd() == 1) {
                areCoPrimeNumbers = true;
            }
        }
